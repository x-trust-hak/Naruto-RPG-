...
async function startBot(phoneNumber, socket = null) {
    ...
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            if (!m.message) return;
            // Block echoed fromMe messages EXCEPT when owner texts their own bot number
            ...
            console.log(`[CMD] from=${from} text=${cleanText}`);
...
async function restoreAllSessions() {
    ...
    try {
        ...
        for (const key of keys) {
            ...
            // Only restore if creds exist (completed pairing)
