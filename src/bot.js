...
const connections = new Map();   // phoneNumber -> WASocket
...
function rollClan() {
    ...
    if (r < 35)   return CLANS[Math.random() > 0.5 ? 2 : 3]; // Hyuga or Aburame
    return CLANS[Math.random() > 0.5 ? 0 : 1]; // Nara or Akimichi
}
...
async function startBot(phoneNumber, socket = null) {
    ...
    // ✅ FIX 1: Prevent duplicate socket instances for the same number.
    // If a live connection already exists, don't spin up a second one.
    if (connections.has(phoneNumber)) {
        const existing = connections.get(phoneNumber);
        // If the existing socket is in a usable state, bail out early
        if (existing?.ws?.readyState === 1 /* OPEN */) {
            console.log(`[BOT] Session ${phoneNumber} is already connected. Skipping duplicate start.`);
            if (socket) socket.emit('connected', 'Already active.');
            return existing;
        }
        // Otherwise it's a dead socket — clean it up and proceed
    ...
    // ✅ FIX 2: Track auth state properly.
    // isSessionAuthenticated gates DB credential writes.
    let isSessionAuthenticated = !!state.creds.me; // true if we loaded real creds from DB
    ...
    const conn = makeWASocket({
        ...
        // ✅ FIX 3: Don't try to re-use a logged-out cached account
    ...
    // ✅ Request pairing code AFTER the WS transport is open but BEFORE auth completes.
    // Baileys signals this via connection.update where connection is undefined/null
    // but ws.readyState is OPEN (1). The safest reliable trigger in Baileys is to
    // simply wait for the FIRST connection.update event of any kind, then delay
    // long enough for the noise handshake to finish (~3-4s), then request.
    // DO NOT request on 'connecting' (too early) or 'open' (too late — already authed).
    const requestPairingCode = async () => {
        ...
        // Wait for the Noise protocol handshake to complete inside Baileys
        ...
        try {
            ...
            if (socket) socket.emit('error', 'Failed to generate pairing code. Please try again.');
        }
    };
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        // Trigger pairing code request on the very first update (before 'open')
        if (connection === 'connecting' && !pairingCodeRequested) {
            requestPairingCode(); // non-blocking, waits 5s internally
        }

        if (connection === 'open') {
            console.log(`✅ [CONNECTED] Session ${phoneNumber} authenticated!`);
            isSessionAuthenticated = true;

            // ✅ FIX 5: Save creds immediately on open so they persist for reconnects
            await saveCreds();
            ...
            try {
                await conn.sendMessage(`${phoneNumber}@s.whatsapp.net`, {
                    text: `🦊 *Naruto RPG connected!* ⚡ System online and ready.`
                });
            } catch (msgErr) {
                console.error(`[BOT] Could not send boot DM to ${phoneNumber}:`, msgErr.message);
            }
        }
            ...
            if (reason === DisconnectReason.loggedOut) {
                console.log(`🗑️ [PURGING] Removing session data for ${phoneNumber}...`);
                try {
                    await clearSession();
                    await User.deleteOne({ phoneId: `${phoneNumber}@s.whatsapp.net` });
                } catch (dbErr) {
                    console.error('[BOT] Error during session purge:', dbErr.message);
                }
                if (socket) socket.emit('logged-out');

            } else if (reason === DisconnectReason.badSession) {
                // ✅ FIX 6: Bad/corrupt session — wipe it from DB then reconnect fresh
                console.log(`⚠️ [BAD SESSION] Clearing corrupt session for ${phoneNumber} and restarting...`);
                await clearSession();
                setTimeout(() => startBot(phoneNumber, socket), 3000);

            } else {
                // Normal transient disconnect — reconnect and restore
                console.log(`🔄 [RECONNECTING] ${phoneNumber} in 7 seconds...`);
                setTimeout(() => startBot(phoneNumber, null), 7000);
            }
        }
    });

    // ✅ FIX 7: Only persist creds after the session is properly open
    ...
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            // ── Auth gate for all other commands ─────────────────────────────
            ...
            if (lowerText === '!profile') {
                const card =
                    ...
                    `💎 *Gems:* ${user.gems}\n\n` +
                    `_Type \`!shop\` or \`!missions\` to continue your journey!_`;
            ...
            if (lowerText.startsWith('!mission ')) {
                ...
                if (user.hp.current <= 1) return await conn.sendMessage(from, { text: `🏥 You're too injured to go on missions! (${user.hp.current} HP)` });
            ...
            if (lowerText === '!exam') {
                ...
                if (!examCfg) return await conn.sendMessage(from, { text: '⚡ You have reached the highest exam tier!' });
...
async function restoreAllSessions() {
    ...
    try {
        ...
        // Find sessions that have creds saved (meaning they authenticated before)
