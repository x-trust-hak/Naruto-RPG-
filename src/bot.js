...
async function startBot(phoneNumber, socket = null) {
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            // Tell WhatsApp we are online and ready to receive messages
    ...
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            console.log(`[MSG] upsert type=${chatUpdate.type} count=${chatUpdate.messages?.length}`);
            if (chatUpdate.type !== 'notify') return;

            const m = chatUpdate.messages[0];
            console.log(`[MSG] from=${m.key?.remoteJid} fromMe=${m.key?.fromMe} hasMsg=${!!m.message}`);
            // Allow fromMe messages — owner messages their own bot number
            if (!m.message) return;
