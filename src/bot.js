...
async function startBot(phoneNumber, socket = null) {
    ...
    // LIFECYCLE HANDLER & SELF-HEALING RECONNECTION MATRIX
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            // 💬 Send live confirmation DM directly to the active session number on reboot
            try {
    ...
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            // 📍 FIX: Separate Group/DM Chat Context from Actual Sender Identity
            const from = m.key.remoteJid; 
            const sender = m.key.participant || m.key.remoteJid; 

            // 📦 FIX: Unwrap Ephemeral/View-Once Structural Containers Safely
            const msg = m.message.ephemeralMessage?.message || 
                        m.message.viewOnceMessage?.message || 
                        m.message.viewOnceMessageV2?.message || 
                        m.message;

            // 🗣️ FIX: Extract Text Strings cleanly from the unwrapped payload
            ...
            // 📡 RADAR DIAGNOSTIC LOGS: Viewable in your Render Dashboard
            console.log(`📥 [MSG] Chat: ${from} | Sender: ${sender} | Command: "${cleanText}"`);

            // 🎯 FIX: Query user records by their UNIQUE SENDER ID, not the from JID
            let user = await User.findOne({ phoneId: sender });
            ...
            if (!user || user.registrationStep !== 'COMPLETED') {
                if (['!profile', '!shop', '!summon', '!donate', '!train', '!missions', '!exam', '!fight', '!pvp'].some(cmd => lowerText.startsWith(cmd))) {
                    console.log(`⚠️ [BLOCKED] ${sender} tried running command without complete registration status.`);
...
// 🛡️ RECOVERY MATRIX ENGINE: Reconnects all sessions saved in MongoDB on app startup
