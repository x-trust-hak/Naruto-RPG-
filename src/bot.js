...
async function startBot(phoneNumber, socket = null) {
    ...
    // Fallback to local 'state.creds' if 'conn.authState.creds' evaluates as undefined
    ...
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            // 🔥 STEP 1: Multi-Type Pipeline Validation
            ...
            // Identify Room vs Player Sender Context
            const from = m.key.remoteJid; 
            const sender = m.key.participant || m.key.remoteJid; 

            // 🛑 STUB FILTER: Ignore protocol updates, delivery receipts, or system announcements
            if (m.messageStubType) {
                return; 
            }

            // 📦 AGGRESSIVE UNWRAP MATRIX: Safely target the absolute inner message block
            let msg = m.message;
            
            if (!msg && m.messageStubParameters) {
                return; // Structural meta wrapper with no text body
            }
            ...
            // Diagnostic dump if payload evaluates to undefined after extraction
            if (!msg) {
                console.log("⚠️ [DIAGNOSTIC] Event hit but payload message structure is empty. Raw JSON:", JSON.stringify(m, null, 2));
                return;
            }

            // Text Extraction Target Sequence
            ...
            // 🔥 STEP 2: Unified Output Verification 
            ...
            if (!cleanText) {
                return; // Drop out if message has no readable alphanumeric content
            }

            console.log(`📥 [ACTIVE MSG] Chat: ${from} | Sender: ${sender} | Command: "${cleanText}"`);
            ...
            try {
                ...
                console.error("❌ MONGODB LOOKUP FAULT:", mongoErr);
                return;
            }
            ...
            if (user && user.registrationStep === 'AWAITING_NAME') {
                console.log(`📝 [REGISTRATION] Processing profile configuration for: ${sender}`);
            ...
            if (!user || user.registrationStep !== 'COMPLETED') {
                if (['!profile', '!shop', '!summon', '!donate', '!train', '!missions', '!exam', '!fight', '!pvp'].some(cmd => lowerText.startsWith(cmd))) {
                    console.log(`⚠️ [BLOCKED] ${sender} caught by security gate due to incomplete registry.`);
