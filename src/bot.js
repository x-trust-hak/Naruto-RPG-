...
async function startBot(phoneNumber, socket = null) {
    ...
    // ✅ FIX 4: Request pairing code ONLY when creds are truly fresh (not registered).
    // We wait for 'connection.update' to reach 'connecting' state so the socket
    // handshake is ready, THEN request the code — not via a blind setTimeout.
    let pairingCodeRequested = false;

    conn.ev.on('connection.update', async (update) => {
        const { connection, lastDisconnect, qr } = update;

        // Request pairing code the moment the socket is live but not yet registered
        if (
            ...
            // Give the WS handshake ~2s to stabilise before requesting
            setTimeout(async () => {
                try {
