...
module.exports = async (conn, from, senderJid, cleanText, phoneNumber, pushName = 'Ninja') => {
    try {
        ...
        if (activeFights.has(senderJid) && activeFights.get(senderJid).turn === senderJid) {
            ...
            const isP1     = senderJid === fight.p1.jid;
        ...
        switch (command) {
            ...
            case 'start': {
                if (user && user.registrationStep === 'COMPLETED') {
                    return await conn.sendMessage(from, {
                        text: `❌ Already registered as *${user.username}* in Hidden ${user.village} Village!\n\n_Type !profile to view your stats._`
                    });
                }
                ...
                const welcomeCard   =
                    ...
                    `💰 Ryo: 1,000 | 💎 Gems: 5\n\n` +
                    `🏯 *Village Group:*\n👉 ${groupInvite}\n\n` +
                    `_Type !setname YourName to change your ninja name!_\n` +
            ...
            case 'setname': {
                ...
                const newName = cleanText.slice(9).trim(); // get original case from cleanText
            ...
            case 'help': {
                return await conn.sendMessage(from, {
                    text: `🍥 *NARUTO RPG COMMANDS* 🍥\n\n` +
                          ...
                          `!setname YourName — Change ninja name\n\n` +
                          `⚔️ *Training & Missions*\n` +
