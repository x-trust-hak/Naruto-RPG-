...
module.exports = async (conn, from, senderJid, cleanText, phoneNumber, pushName = 'Ninja') => {
    try {
        ...
        // Use senderJid for DB lookups (works for both DMs and groups)
        let user = await User.findOne({ phoneId: senderJid });
        ...
        // ── PVP ARENA ─────────────────────────────────────────────────────────
        if (activeFights.has(senderJid) && activeFights.get(senderJid).turn === senderJid) {
        ...
        // ── REGISTRATION: AWAITING_NAME is no longer used ────────────────────
        // Names are now taken from WhatsApp pushName automatically
        ...
        switch (command) {
            ...
            case 'start': {
                if (user && user.registrationStep === 'COMPLETED') {
                    return await conn.sendMessage(from, { text: `❌ Already registered as *${user.username}* in Hidden ${user.village} Village!

_Type \`!profile\` to view your stats._` });
                }

                // Use WhatsApp display name instantly — no back-and-forth
                const rolledClan    = rollClan();
                const randomVillage = ['Leaf', 'Sand', 'Mist', 'Cloud', 'Stone'][Math.floor(Math.random() * 5)];
                const ninjaName     = pushName.slice(0, 16) || 'Ninja';
...
` +
                    `_Type \`!setname YourName\` to change your ninja name anytime!_
` +
            ...
            case 'exam': {
                ...
                activeExams.set(senderJid, { nextRank: examCfg.nextRank, correctAnswer: t.a, rewardRyo: examCfg.rewardRyo });
                return await conn.sendMessage(from, {
            ...
            case 'setname': {
                ...
                if (!newName || newName.length < 3 || newName.length > 16) {
                    return await conn.sendMessage(from, { text: '❌ Name must be 3–16 characters.
...
_Type \`!profile\` to see your updated card!_`
                });
            }
            ...
            case 'help': {
                return await conn.sendMessage(from, {
                    text: `🍥 *NARUTO RPG COMMANDS* 🍥\n\n` +
                          ...
                          `\`!setname YourName\` — Change your ninja name\n` +
                          `\`!menu\` — Show this menu`
                });
            }
